...
void block_current_on_char(){
	TCB * t = &threads[current];
    //my_print_f("CLosed thread %x", current);
	t->state = WAITING_FOR_CHAR;
}

void block_current_on_timer(int time){
	TCB * t = &threads[current];
    //my_print_f("CLosed thread %x", current);
	t->state = WAITING_FOR_TIMER;
	t->wait_end = time +getTime(); // systimer
	
}

void wake_for_char(){
	my_print_f("UNBLOCK ON CHAR\n");
	int i;
    for(i=0; i < MAX_THREADS; i++){
        TCB *tcb = (TCB*) &threads[i];
		if(tcb->state== WAITING_FOR_CHAR){
	        tcb->state=READY;
		}        
    }
...
void wake_for_timer(){
    ...
    for(i=0; i < MAX_THREADS; i++){
        TCB *tcb = (TCB*) &threads[i];
		if(tcb->state== WAITING_FOR_TIMER && tcb->wait_end<=current_time){
	        tcb->state=READY;
			my_print_f("enough\n");
		}        
    }
